...
async function shivaSwapQuote() {
  ...
  if (j.error) {
    document.getElementById('swap-quote').textContent = j.error;
    document.getElementById('swap-out').value = '';
    return;
  }
  document.getElementById('swap-out').value = fmtAtomic(j.amountOut);
  document.getElementById('swap-quote').textContent = `Price impact: ${j.priceImpact} · Pool ${(j.poolId||'').split('|').map(x=>x.split(':')[1]).join(' / ')}`;
}
...
async function loadAmmPools() {
  ...
  const ammEl = document.getElementById('amm-pools');
  if (ammEl) {
    ammEl.innerHTML = pools.map(p => {
      const [a, b] = [p.token0.split(':')[1], p.token1.split(':')[1]];
      return `<div class="asset-row">
        <div class="asset-icon">◇</div>
        <div class="asset-info"><div class="asset-symbol">${a}/${b}</div>
        <div class="asset-name">Fee ${(p.feeBps/100).toFixed(2)}%</div></div>
        <div class="asset-right"><div class="asset-amount">${fmtAtomic(p.reserve0)}</div></div>
      </div>`;
    }).join('');
  }
