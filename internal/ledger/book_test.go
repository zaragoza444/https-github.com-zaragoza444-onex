...
func TestBookConvertActive(t *testing.T) {
	dir := t.TempDir()
	...
	if conv.Status != "completed" || conv.ToAmount == "" {
		t.Fatalf("unexpected convert %+v", conv)
	}
	from, _ := store.GetAccount("usd-acct")
	if parseHuman(from.Balance) != 5000 {
		t.Fatalf("expected 5000 USD left got %s", from.Balance)
	}
	to, _ := store.GetAccount("book:BTC")
	if parseHuman(to.Balance) <= 0 {
		t.Fatalf("expected btc vault credit got %s", to.Balance)
	}
}

func TestBookTransferPreview(t *testing.T) {
	dir := t.TempDir()
	store := NewBookStore(dir)
	store.data = &Book{Accounts: map[string]*BookAccount{
		"usd-acct": {ID: "usd-acct", Asset: "USD", Balance: "10000", Source: SourceBank, Mode: ModeBank},
	...
	res, err := store.Transfer(TransferRequest{
		FromAccount: "usd-acct",
		Amount:      "100",
		ExternalTo:  "ethereum:0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb0",
		ConvertTo:   "ETH",
		Preview:     true,
	}, prices, nil)
	if err != nil {
	...
	if res.Status != "preview" || res.External == nil {
		t.Fatalf("unexpected preview %+v", res)
	}
	from, _ := store.GetAccount("usd-acct")
	if parseHuman(from.Balance) != 10000 {
		t.Fatalf("preview must not debit balance, got %s", from.Balance)
	}
}
